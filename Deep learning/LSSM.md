...
3. B实验设置 
	...
	3. 评判细节（公式） 
4. C对比实验
	1. 分成CNN Transformer mamba 24年三种架构一共找10个左右，先跑点根据跑的点选
...
34. 105827 1486 v14 
35. 105832 1485 v14 把边缘mlp去掉了
36. 105834 1577 v14 把解码器的mlp也去掉了
接下来的工作：
37.  Adam修改成AdamW 引入5e-4
38. 目前去掉AB边界增强效果会变差，第三层变多会变差
	1. 边界信息越浅层越有效
	2. 把v10 的边界增强只留AB原始
39. 把编码器头的可学习参数改成类似于SEB的结构
40. 目前跑的点都是在90左右浮动意义不太大，先把任务完成吧，看当前解码器当中的vssb的结构，把增强分支的mamba改一改
## 消融

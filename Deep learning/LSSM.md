...
3. B实验设置 
	...
	3. 评判细节（公式） 
4. C对比实验
	1. 分成CNN Transformer mamba三种架构一共找10个左右，先跑点根据跑的点选
	2. 三张对比数据集的图，一个数据集选4张图 结果需画图 画成红绿标记图
	3. 三张对比的指标表
	4. 对比参数量计算量表
5. D消融实验 （是要对比不同的设计，还是仅仅对比去掉这块，去掉那块的区别？）
	1. 编码器 共享权重，直接两个分支 三组
	2. 解码器 双vss channel spatial (quan)四组
	3. 增强 一阶 二阶 Mamba (全无 留一阶 留二阶 都不留 全留 五组)
6. 
...
2. 目前去掉AB边界增强效果会变差，第三层变多会变差
	1. 边界信息越浅层越有效
	2. 把v10 的边界增强只留AB原始
3. 把编码器头的可学习参数改成类似于SEB的结构
4. 目前跑的点都是在90左右浮动意义不太大，先把任务完成吧，看当前解码器当中的vssb的结构，把增强分支的mamba改一改
## 消融

...
59. 106577 1577 v16 移除了再跑一次 掉大点
60. 106578 1486 按45再跑 v15 0.01
61. 106579 1485 按45再跑 v15 0.01 掉点 这一轮可以用来做消融 因为取得t300
接下来的工作：
62.  Adam修改成AdamW 引入5e-4
63. 目前去掉AB边界增强效果会变差，第三层变多会变差
	1. 边界信息越浅层越有效
	2. 把v10 的边界增强只留AB原始
64. 把编码器头的可学习参数改成类似于SEB的结构
65. 目前跑的点都是在90左右浮动意义不太大，先把任务完成吧，看当前解码器当中的vssb的结构，把增强分支的mamba改一改
...
1.深度学习 把测试集数据改成验证集，从而再训练时直接根据测试集指标来训练模型
## 消融

...
54. 106394 1577 v16 把编码器的合并改成非dwconv了，加了bn relu 1e-3
55. 106396 1486 v16 编码器前面改成共享权重 合并部分调回原来的了 1e-3  训练的好像不是共享权重，但跟代码对应不上
56. 106460 1485 v16 编码器共享CNN 1e-3
接下来的工作：
57.  Adam修改成AdamW 引入5e-4
58. 目前去掉AB边界增强效果会变差，第三层变多会变差
	1. 边界信息越浅层越有效
	2. 把v10 的边界增强只留AB原始
59. 把编码器头的可学习参数改成类似于SEB的结构
60. 目前跑的点都是在90左右浮动意义不太大，先把任务完成吧，看当前解码器当中的vssb的结构，把增强分支的mamba改一改
## 消融

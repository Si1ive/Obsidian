...
本发明公开了一种基于Mamba模型的遥感图像变化检测方法，包括将遥感图像的变化检测任务解耦为双时图像建模、差异特征捕捉以及目标物体重建三个子任务；搭建基于Mamba的时相交互式的编码模块；搭建带有双阶导数边缘算子和频率自适应Mamba模块的差异模块；搭建Mamba序列建模模式协同空间-通道注意力的解码模块；利用UNet架构联合三个子任务模块；采用交叉熵与Lovasz-softmax 的联合损失函数训练网络。本发明本利用Mamba模型,通过任务解耦,增强网络全局建模能力同时关注任务特性,与传统络相比，得到了更好的分类精度，在遥感图像变化检测领域中具有重要的应用价值。

...
淘宝图片数据存在问题
1. 包含无关格式数据，如：视频，.txt，.pdf，.ai，.psd登格式
2. 图片存在水印，分辨率低不清晰等质量问题
3. 图片以压缩包形式存储，部分压缩包存在解压密码
4. 存在空文件夹
5. 海报需要中文和英文内容，但存储有日文内容的
6. 图片内容是影印版非电子版，特点如：漏出纸质图片的边角
7. 插画素材中可能存在暴力等违规内容
8. 存在大量文字或纯文字内容图片
...
3. 双时图中体现的不明显，老师倾向于分离合并，如果起这个，那应该把线画的更明显点
4. mamba取黄色，左边就不应该是黄色
...
39. 105942 1486 v14 16 1E-3
接下来的工作：
40.  Adam修改成AdamW 引入5e-4
41. 目前去掉AB边界增强效果会变差，第三层变多会变差
	1. 边界信息越浅层越有效
	2. 把v10 的边界增强只留AB原始
42. 把编码器头的可学习参数改成类似于SEB的结构
43. 目前跑的点都是在90左右浮动意义不太大，先把任务完成吧，看当前解码器当中的vssb的结构，把增强分支的mamba改一改
## 消融
